...
Dockerfile / .dockerignore   Production container (Bun + Litestream) — see docs/runbooks/deploy.md
.railway/railway.ts          Railway infrastructure-as-code: service, /data volume, region, health check, 1 replica, non-secret env. Replaced fly.toml (WS-B2). `railway config plan` previews, `apply` converges
tests/
...
| Hosting | **Railway** Bun container + SQLite on a /data volume + Litestream (launch plan D10 as amended by WS-B2) — live since 2026-09-09. Cloudflare Pages keeps serving the static export as the read-only fallback | The paid tier needs per-user serving; one box runs the code as written |
...
- The production server is **deployed on Railway** (2026-09-09) and serving over HTTPS with the full security-header set, but it is **not yet the public site**: there is no product domain (D2/A6) and no replica bucket, so Litestream is **not replicating** and the public URL is still the static Pages export. The CI → `--no-deploy` flip (D18) stays deferred until a refresh run on the server is verified (see the WS-B plan's "Sequencing"). The volume is 500 MB — Railway's Trial cap — against the 3 GB WS-B2 specifies, so the server database is currently a bounded Cup backfill rather than the full three-series archive
