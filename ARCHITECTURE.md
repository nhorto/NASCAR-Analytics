...
- **The mobile app is a scaffold, not a product** (WS-J stages 1–2 only): no IAP (RevenueCat needs owner steps J1/J2), no native push (J5), no predictions/DFS/deep-tool content screens (locked/“coming” states only), no races index or recap screens (those pages have no JSON API yet), no EAS/store config, and it has never run on a physical device. Sign-in relies on the platform cookie jar following the server's PRG redirects; if a real device breaks that assumption the fallback is the additive native-session endpoint named in the WS-J sub-plan
