...
│   ├── layout.ts            Page shell — responsive (2026-09-09 UX realignment): <900px = app bar + series switch + a 5-tab bottom bar (Home/Live/Picks/Stats/Account) matching the native app; ≥900px = a grouped desktop sidenav. One HTML for all widths; a media query in style.css picks. Inline-SVG nav icons via html.ts navIcon(). 404 here too.
...
│   └── pages/               Page templates (home, drivers, races, metrics, career, recap, predictions, dfs, stats-hub) + client shells (compare, tracks) + auth/account/pricing/unsubscribe forms and the Pro teaser (WS-D/F/G). `stats.ts` is the `/stats` hub — the mobile Stats tab's landing, linking every section the 5-tab bar demoted.
├── domains/
...
├── src/app/                 expo-router routes (thin wrappers over feature screens). Five tabs (2026-09-09 UX realignment): Home · Live · Picks · Stats · Account, Ionicons, red dot on Live when a race is on track. NO "Pro" tab — Pro is inline lock states + one shared UpsellSheet. picks.tsx hosts a Predictions ⇄ DFS segment; stats.tsx is a hub + boards; predictions/dfs/drivers stay registered as href:null deep-link targets
├── src/features/<name>/     api.ts (fetch + defensive parse) / model.ts (pure view-model) / screens; api+model never import react-native, tests colocated. features/pro/ holds ProLock (in-screen locked card), UpsellSheet (the shared sheet + UpsellProvider/useUpsell) and SeriesPills (Cup/Xfinity/Trucks, locked for free viewers → upsell)
├── src/lib/                 server base-url config, timeout fetch, cookie/auth flows, viewer/entitlement context (viewer.tsx), series context (series.tsx), foreground-only live-status poller (liveStatus.tsx), stubbed purchases.ts + push.ts (owner-gated J1/J2/J5)
...
- **The mobile app has content but no commerce and no device proof** (WS-J stages 1–2; the Pro content screens landed 2026-09-08; the nav realignment 2026-09-09): no IAP (RevenueCat needs owner steps J1/J2), no native push (J5), no EAS/store config. It **has now been run and driven on both an iOS simulator and an Android emulator** (2026-09-09, Maestro, both entitlement states — the first Android run in the project; see [the drive report](docs/exec-plans/completed/2026-09-09-mobile-device-drive.md)), which found and fixed a **P1** sign-in defect that locked every unverified account out of the app. Still never seen rendered: the Live tab's red dot (no race was on track) and the live board during an actual race. The app is now Home · Live · Picks · Stats · Account with no Pro tab, and **Stats/Drivers browse all three series** (Cup free; Xfinity/Trucks Pro, via the existing `?series=` endpoints; free viewers get locked pills → the upsell sheet). Still absent from the app: **Xfinity/Trucks in the live board** (Cup-fixed by design, D16) and in Picks (no non-Cup predictions data yet), CSV export (a download opens outside the app and leaves the session cookie behind), and races-index/recap screens (those pages have no JSON API). Sign-in relies on the platform cookie jar following the server's PRG redirects; if a real device breaks that assumption the fallback is the additive native-session endpoint named in the WS-J sub-plan
