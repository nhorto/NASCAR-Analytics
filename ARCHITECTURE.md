...
│   ├── static/brand/        Brand raster assets: the unlettered tire master (900px JPEG, from the V3 brand exploration) that the landing page letters via SVG
│   ├── static/shots/        Real product screenshots embedded by the landing page (list = LANDING_SHOTS in pages/welcome.ts; recapture recipe in the exec plan)
│   └── pages/               Page templates (home, drivers, races, metrics, career, recap, predictions, dfs) + client shells (compare, tracks) + auth/account/pricing/unsubscribe forms, the Pro teaser (WS-D/F/G), and the standalone marketing landing page (welcome.ts, served at /welcome)
├── domains/
...
- **Marketing landing page at the site root (2026-09-08, v2 after owner review)**: `/` serves a standalone conversion-focused marketing document to **anonymous** visitors; signed-in viewers get the app home at the same URL, and the app home also lives at **`/home`** (any series prefix) so signed-out browsing still works — the Home tab, wordmark, and 404 links now point at `/home`, and the static export writes the landing at `/` with `${prefix}/home` homes (bare `/xfinity`/`/trucks` keep home copies for old URLs; `/welcome` stays as an alias). Copy funnel per the exec plan: hero = the free live race-day companion, then the proprietary metrics, then Pro predictions/DFS/deep tools, Free-vs-Pro pricing cards (spec §4 numbers), and an app coming-soon section; CTAs drive `/signup` and `/pricing`. Section visuals are **real product screenshots** captured from the running app against the live database (`static/shots/`: predictions with the free-top-3 teaser, adjPE leaderboard, Larson profile, Southern 500 recap) plus a live-board mock built from the app's design idioms (pit window, tire severity, My Driver alert). The hero logo is the **unlettered** V3 tire master with the working name ("Looplab") drawn as SVG curved text — the eventual real name is a one-constant change in `pages/welcome.ts`. Carries the NASCAR non-affiliation and not-gambling-advice disclaimers; holds the CSP bar (boot.js only); covered by `tests/app.welcome.test.ts` + the CSP page sweep.
