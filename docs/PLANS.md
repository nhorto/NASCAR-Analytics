...
| [Strategy Model Calibration](exec-plans/active/2026-07-06-strategy-model-calibration.md) | ACTIVE | Replace the live Strategy tab's fake `DEFAULT_STINT_LAPS=40` + raw-speed proxy with per-track tire/fuel/pit constants fit from the historical backfill (`track-strategy.json`, baked like `baselines.json`). Backed by [research](research/2026-07-06_tire-fuel-strategy-data-and-modeling.md) (no feed carries tire/fuel — must model) + a 2026-07-06 spike. **Phases 1–3 DONE + deployed (2026-07-06):** calibrated all 3 series locally, replaced the fuel-burn-confounded within-stint falloff with the **pit-discontinuity** tire-severity method, reframed the fuel window to a behavioral **typical green run** (a physical fuel capacity isn't recoverable from history), baked it series-keyed into the worker + shipped an honest Strategy UI (tire tier, low-deg suppression). Only Phase 4 (held-out backtest / error bars) remains. |
