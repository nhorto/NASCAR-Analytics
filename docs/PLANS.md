...
| [Strategy Model Calibration](exec-plans/active/2026-07-06-strategy-model-calibration.md) | PROPOSED | Replace the live Strategy tab's fake `DEFAULT_STINT_LAPS=40` + raw-speed proxy with per-track tire/fuel/pit constants fit from the historical backfill (`track-strategy.json`, baked like `baselines.json`). Backed by [research](research/2026-07-06_tire-fuel-strategy-data-and-modeling.md) (no feed carries tire/fuel — must model) + a 2026-07-06 spike proving the pit-feed extraction method (and that calibration needs many races/track, so Phase 1 is local-only). |
