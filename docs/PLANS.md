...
|------|--------|-------------|
| _None right now_ | | |
| _Other candidates_ | | Connect the Cloudflare deploy + add the two CI secrets (owner login — unblocks weekly auto-deploys of site + Worker), background Web Push (VAPID + DO subscription store; the step toward a "real app"), app-store packaging (TWA/Capacitor, after PWA traction), SEO/OpenGraph metadata + sitemap, cross-series statistical comparison, sharper metric baselines (era/track-type). |

## Completed Plans

| Plan | Completed | Description |
|------|-----------|-------------|
| [PWA: Installable + Notifications](exec-plans/completed/2026-07-18-pwa-installable.md) | 2026-07-18 | The site is now an installable PWA: `manifest.webmanifest` + generated icon set (`scripts/gen-icons.ts`, committed PNGs), PWA head metas, and a service worker (offline shell + stale-while-revalidate assets, cache keyed to the content-hash `ASSET_VERSION`; cross-origin live API + `/api/*` never intercepted). Opt-in 🔔 device notifications for the followed driver (+ caution/restart) on the Live page while Looplab is open/backgrounded. Background Web Push logged as a roadmap candidate. |
| [Live Race Day Companion](exec-plans/completed/2026-07-05-live-race-companion.md) | 2026-07-18 | Free, mobile-first live race companion (owner-approved 2026-07-05): the pure Workers-safe `live` domain + the `looplab-live` Worker/`LiveCoordinator` DO ([looplab-live.nhorton.workers.dev](https://looplab-live.nhorton.workers.dev)) polling the NASCAR CDN live feed, and the main-site `/live` page (layered board, Loop Rating ★, Overview/Strategy/My Driver, home 🔴 banner). Phases 0–3 shipped + live-validated 2026-07-05; **Phase 4 hardening 2026-07-18**: race-window keep-warm cron (`anyRaceInWindow`, start − 30 min → + 6 h) and the post-race authoritative `loopstats/prod` swap (`applyAuthoritativeStats`, payload `authoritative: true`) — unit-tested; live validation at the next race weekend post-deploy. |
